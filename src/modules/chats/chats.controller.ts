...
export class ChatsController {
    ...
    async getAllLobby(@Req() request: any) {
        ...
        return await this.chatsService.getAllLobby(id);
    }
    ...
    async getAllRooms(@Req() request: any) {
        const {id} = request['user'];
        return await this.chatsService.getAllRooms(id);
    }
    ...
    async createRoom(@Req() request: any, @Param() params: any) {
        const {roomId} = params;
        const {id} = request['user'];
        return await this.chatsService.createRoom(id, roomId);
    }
    ...
    async createLobby(@Req() request: any, @Param() params: any) {
        const {roomId} = params;
        const {id} = request['user'];
        return await this.chatsService.createLobby(id, roomId);
    }
    ...
    @ApiParam({name: 'roomId', type: String})
    async createRoomMessages(@Req() request: any,
                             @Param() params: any,
                             @Body() body: SendMessageDTO,) {
        const {roomId} = params;
        const {id} = request['user'];
        const {message} = body;
        return await this.chatsService.createMessage(roomId, id, message, 'ROOM');
    }
    ...
    @ApiParam({name: 'roomId', type: String})
    async createLobbyMessages(@Req() request: any,
                              @Param() params: any,
                              @Body() body: SendMessageDTO,) {
        const {roomId} = params;
        const {id} = request['user'];
        const {message} = body;
        return await this.chatsService.createMessage(roomId, id, message, 'LOBBY');
    }
}
