...
export class AutoGenerated1708513775203 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `CREATE TYPE game_type_enum AS ENUM ('PUBLIC', 'PRIVATE');`,
    );
    await queryRunner.query(
      `CREATE TYPE game_status_enum AS ENUM ('INGAME', 'WAITING');`,
    );
    await queryRunner.query(
      `CREATE TYPE room_type_enum AS ENUM ('PUBLIC', 'PRIVATE');`,
    );
    await queryRunner.query(
      `CREATE TYPE room_status_enum AS ENUM ('INGAME', 'WAITING');`,
    );
    await queryRunner.query(
      `CREATE TYPE chat_chat_type_enum AS ENUM ('LOBBY', 'ROOM');`,
    );
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(`DROP TYPE IF EXISTS chat_chat_type_enum;`);
